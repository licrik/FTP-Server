...
namespace FTP_Server
{
    public class ClientConnection : IDisposable
    {
                    ...
                    if (response == null)
                    {
                        switch (cmd)
                        {
                            case "USER":
                                ...
                                break;
                            case "PASS":
                                response = Password(arguments, _clientIP);
                                break;
                            case "CWD":
        ...
        {
            Program.DataSender("Connection login: " + username);
            _username = username;

            return "331 Username ok, need password";
        }

        private string Password(string password, string ip)
        {

            _currentUser = UserStore.Validate(_username, password);

            Program.DataSender("current_user: " + _currentUser);
            ...
            if (_currentUser != null)
            {
                ...
                Program.DataSender("Successful login ip = " + ip);
        ...
        private string PrintWorkingDirectory()
        {
            ...
            string current = _currentDirectory;
        ...
        private string Rename(string renameFrom, string renameTo)
        {
            ...
            if (renameFrom != null && renameTo != null)
            {
                ...
                Program.DataSender("Rename " + renameFrom);
        ...
        private string RetrieveOperation(NetworkStream dataStream, string pathname)
        {
            ...
            Program.DataSender("Client download " + pathname);
        ...
        static public string ListOperation(NetworkStream dataStream, string pathname)
        {
            ...
            foreach (string dir in directories)
            {
                ...
                string date = d.CreationTime.ToString("MM-dd-yy hh:mmtt");
                string strFilesList = date + " <DIR> " + d.Name + "\r\n";
                //string line = string.Format("-rw-r--r--  {0}", d.Name);

                dataWriter.WriteLine(strFilesList);
                dataWriter.Flush();
            }
            ...
            foreach (string file in files)
            {
                ...
                string date = f.CreationTime.ToString("MM-dd-yy hh:mmtt");

                string strFilesList = date + " " + f.Length.ToString() + " " + f.Name + "\r\n";
        ...
        static public string ListOperation( string pathname)
        {
            IEnumerable<string> directories = Directory.EnumerateDirectories(pathname);
            ...
            foreach (string dir in directories)
            {
                ...
                string line = string.Format(d.Name);
            }
        ...
        public static string CallBack(string message)
        {
            return message;
        }
    }
